...
def start_threads():
    ...
    print("start_threads", global_do_loop, global_id_info_loop, do_parse, abort)
    ...
    if t_parse_thread is None or not t_parse_thread.is_alive():
        t_parse_thread = threading.Thread(target=threaded_parse, args=(15,))
        t_parse_thread.start()
    
    if t_timer_execute is None or not t_timer_execute.is_alive():
        t_timer_execute = threading.Thread(target=timer_execute, args=(stop_threads, 300))
...
def stop_threads():
    ...
    global global_do_loop, global_id_info_loop, do_parse, abort
    print("stop_threads", global_do_loop, global_id_info_loop, do_parse, abort)
    global_do_loop = False
    global_id_info_loop = False
    do_parse = False
    abort = True
